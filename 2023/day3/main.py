...
def get_engine_parts(
    symbol_locs: List[Tuple[int, int]],
    digits: List[Tuple[int, Tuple[int, int], int]]
) -> List[int]:
    return [
        num
        for row, span, num in digits
        for loc in symbol_locs
        if (abs(row - loc[0]) < 2) and ((abs(span[0] - loc[1]) < 2) or (abs((span[1] - 1) - loc[1]) < 2))
    ]
    ...
    for gear_row, gear_col in gear_locs:
        ...
        for num_row, num_span, num in digits:
            if abs(num_row - gear_row) > 1:
                continue
            if abs(num_span[0] - gear_col) > 1 and abs((num_span[1] - 1) - gear_col) > 1:
                continue
            nums.append(num)
