...
def create_map(input_str: str) -> dict[str, dict[str, str]]:
    ...
    matches = [MAP_REGEX.match(s) for s in input_str]

    mapping = {m.group(1): {"L": m.group(2), "R": m.group(3)} for m in matches}
...
def get_ghost_steps(instructions: str, map_dict: dict[str, dict[str, str]]) -> int:
    current_locs = [x for x in map_dict.keys() if x[2] == "A"]

    steps = [get_steps(instructions, map_dict, loc, r"\w\wZ") for loc in current_locs]
